...
def mutation(tensor, mutationRate,mutationTax):
    ...
    with tf.name_scope('Mutation'):
        ...
        mutation_multiplied = mutationRate * 100
        ...
        comparison = tf.math.greater( random_array_binary, tf.constant( mutation_multiplied ) )
...
def mutation_by_node(tensor, mutationRate):
    ...
    with tf.name_scope('Mutation'):
        ...
        shapeSize = tf.shape(tensor)
        random_array_binary =  tf.random_uniform(dtype=tf.float32, minval=0, maxval=100, shape=[shapeSize[0],shapeSize[1]])
