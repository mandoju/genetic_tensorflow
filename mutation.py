...
def mutation(neural_networks, mutationRate):
    # depois fazer matrix mascara (a.k.a recomendacao do gabriel)
    with tf.name_scope('Mutation') as scope:
        ...
        population_size = len(neural_networks)
        ...
        for i in range(population_size):
            ...
            with tf.name_scope('Passagem_Genes') as scope:
                ...
                for weight_idx in range(len(mother_tensor)):
                    ...
                    # mother_tensor_process = mother_tensor[weight_idx];
                    # father_tensor_process = father_tensor[weight_idx];

                    tensor_process = mother_tensor[weight_idx];
                    ...
                    random_array_binary = tf.cast(
                        tf.random_uniform(dtype=tf.float32, minval=0, maxval=1, shape=[shape_size[0], shape_size[1]]),
                        tf.float32)
                    ...
                    mutated_weight_tensor = tf.Variable( tf.multiply(tensor_process, random_array_start[:, tf.newaxis]) );

                    temp_neural_network.append(mutated_weight_tensor[:]);
