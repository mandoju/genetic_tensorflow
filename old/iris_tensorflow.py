...
# Data sets
IRIS_TRAINING = "./input/iris_2.csv"
IRIS_TEST = "./input/iris_2.csv"
IRIS_DATA_SIZE = 4
CLASS_SIZE = 3
...
class Classifier:
    ...
    def inference(self, x):
        ...
        with tf.name_scope("input"):
            weights = tf.Variable(tf.truncated_normal([IRIS_DATA_SIZE, self._hidden_units[0]],
                                                      stddev=get_stddev(IRIS_DATA_SIZE, self._hidden_units[0])),
                                  name='weights')
        ...
        for index, num_hidden in enumerate(self._hidden_units):
            ...
            with tf.name_scope("hidden{}".format(index + 1)):
                weights = tf.Variable(tf.truncated_normal([num_hidden, self._hidden_units[index + 1]],
                                                          stddev=get_stddev(num_hidden, self._hidden_units[index + 1])),
                                      name='weights')
        ...
        with tf.name_scope('output'):
            weights = tf.Variable(tf.truncated_normal([self._hidden_units[-1], self._n_classes],
                                                      stddev=get_stddev(self._hidden_units[-1], self._n_classes)),
                                  name='weights')
