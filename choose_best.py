...
def choose_best_tensor_conv(convulations,biases, fitnesses):
    with tf.name_scope('Choose_best') as scope:
        ...
        convulation_weights_output = {key: None for key in convulations_weights_keys}
        biases_output_keys=  list(biases.keys())
        biases_output = {key: None for key in biases_output_keys}

        convulation_weights_best_output = {key: None for key in convulations_weights_keys}
        biases_output_best = {key: None for key in biases_output_keys}

        for key in convulations:
            convulation_weights_output[key] = tf.stack([ convulations[key][top_indices[0]],convulations[key][top_indices[1]] ]);
            convulation_weights_best_output[key] = convulations[key][top_indices[0]]
        for key in biases:
            #print(idx);
            biases_output[key] = tf.stack([ biases[key][top_indices[0]],biases[key][top_indices[1]] ]);
