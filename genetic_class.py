...
class Population:
    ...
    def __init__(self, geneticSettings):
        ...
        self.population, self.populationShape, self.convulations, self.bias = create_population(
            geneticSettings['layers'], geneticSettings['weights_convulation'], geneticSettings['biases'], geneticSettings['populationSize'])
    ...
    def run_epoch(self):
        ...
        best_conv, best_bias, the_best_conv, the_best_bias, mutate_conv, mutate_bias = choose_best(
            self.geneticSettings['selection'], self.neural_networks.convulations, self.neural_networks.biases, fitness, self.eliteSize)

        finish_conv, finish_bias = apply_genetic_operatos(self.geneticSettings['genetic_operators'], self.operatorSize, self.eliteSize,
                                                          self.convulations, self.bias, best_conv, best_bias, self.populationShape, self.populationSize, self.mutationRate, 2, len(self.layers))
        ...
        optimizer = tf.train.AdamOptimizer(
            learning_rate=0.0001).minimize(self.neural_networks.cost[0])
                        ...
                        for population_slice in self.slice_sizes:
                            ...
                            if(population_slice > 1):
                                operators_max.append(
                                    max(cost[last_population_slice:slice_finish]))
                            else:
                                operators_max.append(
